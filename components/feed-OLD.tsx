		...
		const observer = new IntersectionObserver(
			(entries) => {
				if (entries[0].isIntersecting && !loading) {
					setDisplayedItems((prev) => [...prev, ...items.slice(prev.length, prev.length + 5)]);
				}
			},
			{ threshold: 1 },
		);
		...
		<div className="p-3 flex flex-col gap-10">
			...
			<AnimatePresence>
				{displayedItems.map((item) => (
					<motion.div initial={{ opacity: 0, y: 40 }} animate={{ opacity: 1, y: 0 }} key={item.id}>
						<Card className="rounded-2xl">
							...
							<CardFooter className="pt-1">
								<div className="flex flex-col gap-2 w-full">
									...
									<p className="text-neutral-400 italic text-sm">{dayjs(item.timestamp).fromNow()}</p>
								</div>
							</CardFooter>
						</Card>
					</motion.div>
