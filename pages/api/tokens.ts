...
async function handler(
  ...
  if (!address) {
    return res.status(400).json({ tokens: [], error: "Wallet address is required" });
  }
  ...
  try {
    ...
    while (retries > 0) {
      try {
        response = await fetch(`https://mainnet.helius-rpc.com/?api-key=${HELIUS_API_KEY}`, {
          method: 'POST',
    ...
    const tokens = fungibleTokens.map((token: any) => {
      ...
      return {
        mint: token.id,
        amount: tokenInfo.balance !== undefined ? tokenInfo.balance : token.ownership.amount,
