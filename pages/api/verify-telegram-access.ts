...
export default async function handler(
  ...
  try {
    ...
    const telegramPlugin = mapping.items?.find((item: any) => 
      item.type === 'telegram' && item.tokenGated
    );

    if (!telegramPlugin) {
      return res.status(400).json({ error: 'Telegram channel not configured or not token gated' });
    }
    ...
    const userToken = data.result.items.find((item: any) => 
      item.id === requiredToken
    );
    ...
    const userBalance = userToken.token_info?.balance || userToken.ownership?.amount || 0;
